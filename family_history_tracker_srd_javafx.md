...
## 7. Canonical Version 1 Workflow
This section defines the single most important end-to-end Version 1 workflow. It is the reference path for implementation priority, UI design, acceptance testing, and scope decisions.

### Primary Goal
Enable the user to work a family line in a deliberate, keyboard-efficient order:
- start from the root person,
- move backward through ancestors,
- inspect each ancestor's descendant line,
- record person ordinances and spouse-specific sealings,
- understand what is open now versus blocked or opening soon,
- and preserve progress safely through local persistence and backup.

### Canonical Workflow
1. Create or open the root person
   The user begins with self or another selected root person and confirms the starting point for pedigree navigation.
2. Add and maintain family relationships
   The user adds parents, spouses, and children so the pedigree and descendancy structure are complete enough to navigate meaningfully.
3. Navigate backward through ancestors
   The user moves up the pedigree and selects an ancestor line to inspect.
4. Inspect the ancestor's descendant line summary
   The application shows the badge, counts, next-availability timing, and unresolved/open conditions for that line.
5. Open people within the selected line
   The user reviews person detail, relationship context, and spouse links in an ordered workflow.
6. Record ordinance data
   The user records person-level ordinances and records sealed-to-spouse status on each spouse relationship.
7. Evaluate 110-year readiness and near-term horizons
   The application classifies descendant work as open now, opening soon, blocked by 110 years, not applicable, or unknown.
8. Review actionable and exception-focused reports
   The user uses work queue, eligibility, and line/report views to focus on open work, soon-opening work, and unresolved data.
9. Continue with fast keyboard-oriented operation
   Common navigation and status-entry tasks should be possible without requiring heavy mouse use.
10. Preserve work safely
   The user can refresh, export, and back up the local database without relying on network services.

### Version 1 Priority Map
The following capabilities are mandatory because they directly support the canonical workflow:

#### Priority 1: Must Exist for Version 1
- Root person creation and selection
- Person CRUD with names, dates, sex, living/deceased, notes, reviewed status
- Parent-child relationships
- Spouse relationships with spouse-specific sealing status
- Pedigree navigation rooted in the selected person
- Descendancy/ancestor-line inspection
- Person ordinance tracking
- 110-year classification and horizon buckets
- Ancestor line badge computation
- Search and filtering needed to find people and lines
- Keyboard-efficient data entry and status updates
- Local SQLite persistence
- Backup/export workflow

#### Priority 2: Important but Secondary
- Stewardship labeling for lines
- Rich report filtering and saved report views
- Import/export formats beyond core backup needs
- Additional summary caches or optimization layers

#### Priority 3: Future-Facing
- Android companion behaviors
- Sync queues, conflict handling, and remote convergence
- Audit-heavy sync event modeling

### UI Mapping for the Canonical Workflow
- Person table and root selection support workflow steps 1 through 3
- Parent, child, and spouse editors support step 2
- Pedigree view supports step 3
- Ancestor line summaries, reports, and badges support steps 4, 7, and 8
- Person detail and relationship tabs support step 5
- Ordinances and spouse sealing data entry support step 6
- Work queue and eligibility views support steps 7 and 8
- Keyboard shortcuts and selection behavior support step 9
- Backup/export controls support step 10

### Domain Mapping for the Canonical Workflow
- `person` supports identity, dates, reviewed status, and root context
- `parent_child_link` and `spouse_link` support family structure
- `person_ordinance_status` supports person-level ordinances
- `spouse_link.sealing_to_spouse_status` supports spouse-specific sealing work
- `AncestorLineSummary` supports line-level operational awareness
- `LineStewardship` supports optional ownership and monitoring workflows
- ordinance and horizon enums support classification and reporting

### Scope Decision Rule
When deciding whether a feature belongs in Version 1, ask:
- Does it directly improve one of the ten canonical workflow steps?
- Does it make pedigree-indexed descendancy management faster, clearer, or safer?
- Does it preserve the clean architecture needed for future Android/sync support without delaying the core workflow?

If the answer is no, the feature should normally be deferred.

### Acceptance Alignment
Version 1 is successful only if a user can complete the full canonical workflow end to end:
- establish a root person,
- build enough pedigree and descendancy structure to navigate lines,
- record both person ordinances and spouse sealings,
- see ancestor-line status and 110-year readiness,
- work from actionable reports,
- and preserve everything through local persistence and backup.
...
## 10. Business Rules and Domain Logic
### Date Precision
...
## 17. Persistence, Files, Backup, and Sync Strategy
...
## 21. Future Cross-Platform and Sync Requirements
...
## 23. Suggested Initial Prompt for a Coding AI
